...
class DenseDataLoader(Dataset):
    def __init__(self,
        ...
        if self.train:
            read_file_name = '../data/dp_annotation/minival.pkl'

        else:
            read_file_name = '../data/dp_annotation/minival.pkl'  # 'valminusminival'
            self.point_select = 1

        self.iuv_loc = os.path.join(parent_dir, 'noc_minival2014')
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.parent_dir,
                               'densepose_noc',
                               os.path.splitext(internal_file_loc)[0] + '.pkl'), 'rb') as in_file:
            iuv_data = pickle.load(in_file)
        dense_noc = iuv_data['noc']
        # print(background[:, :, 0] == )
        ...
        data_dict['image'], dense_noc, (h_, w_), (c_h, c_w) = self.fixed_rescale_crop(image=image,
                                                                                       background=dense_noc)
        data_dict['dense_noc'] = torch.from_numpy((dense_noc.transpose([2, 0, 1]) > 0).astype('float32'))
